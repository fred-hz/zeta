...
class AlphaFoxtrot_3(AlphaBase):
    def initialize(self):
        ...
        for ii in range(len(self.context.ii_list)):
            if self.is_valid[di][ii]:
                ...
                if count1 >= 10:
                    temp1 = np.zeros(10)
                    for i in range(10):
                        if np.where(-np.isnan(self.low[di-self.delay-np.arange(5), ii]))[0].size == 0:
                            continue
                        temp1[i] = np.nanargmin(self.low[di-self.delay-np.arange(5), ii])
                    self.alpha[ii] = util.corr(temp1, RSI[count1-1-np.arange(10)])
