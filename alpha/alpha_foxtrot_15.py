...
class AlphaFoxtrot_15(AlphaBase):
    def initialize(self):
    ...
    def compute_day(self, di):
        for ii in range(len(self.context.ii_list)):
            if self.is_valid[di][ii]:
                ...
                if count < 10:
                    continue
                self.alpha[ii] = -util.corr(DX[count-1-np.arange(10)], self.high[di-self.delay-np.arange(10), ii] - self.low[di-self.delay-np.arange(10), ii])
