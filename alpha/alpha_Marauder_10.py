...
class AlphaMarauder_10(AlphaBase):
    def initialize(self):
    ...
    def compute_day(self, di):
        for ii in range(len(self.context.ii_list)):
            if self.is_valid[di][ii]:
                ...
                value1 = np.zeros(20)
                for i in range(20):
                    value1[i] = util.EffiRatio(self.ret[di-self.delay-i-np.arange(5), ii])

                if count1 >= 20:
                    self.alpha[ii] = util.corr(value1[np.arange(5)], RSI[count1 - 1 - np.arange(5)])
