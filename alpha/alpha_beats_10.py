        ...
        for ii in range(len(self.context.ii_list)):
            if self.is_valid[di][ii]:
                ...
                for i in range(n-1, -1, -1):
                    if np.isnan(self.high[di-self.delay-i][ii]) or np.isnan(self.high[di-self.delay-i-1][ii]) or np.isnan(self.low[di-self.delay-i][ii]) or np.isnan(self.low[di-self.delay-i][ii]):
                        continue
                    if count == 0:
                        ATR[count] = self.high[di-self.delay-i][ii] - self.low[di-self.delay-i][ii]
                        if self.high[di-self.delay-i][ii] - self.high[di-self.delay-i-1][ii] > self.low[di-self.delay-i-1][ii] - self.low[di-self.delay-i][ii] and self.high[di-self.delay-i][ii] > self.high[di-self.delay-i-1][ii]:
                            PDM[count] = self.high[di-self.delay-i][ii] - self.high[di-self.delay-i-1][ii]
                        if self.high[di-self.delay-i][ii] - self.high[di-self.delay-i-1][ii] < self.low[di-self.delay-i-1][ii] - self.low[di-self.delay-i][ii] and self.low[di-self.delay-i-1][ii] > self.low[di-self.delay-i][ii]:
                        ...
                        ATR[count] = (self.high[di - self.delay - i][ii] - self.low[di - self.delay - i][ii]) * 0.1 + ATR[count-1] * 0.9
                ...
                temp = np.zeros(5)
                for i in range(5):
                    if abs(DX[count-1-i] > 1e-3):
                        temp[i] = (self.high[di-self.delay-i][ii] - self.cps[di-self.delay-i][ii]) / DX[count-1-i]
